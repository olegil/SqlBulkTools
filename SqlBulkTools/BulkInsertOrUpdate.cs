...
namespace SqlBulkTools
{
    public class BulkInsertOrUpdate<T> : ITransaction
    {
        ...
        public BulkInsertOrUpdate(List<T> list, string tableName, string schema, HashSet<string> columns, string sourceAlias, string targetAlias, 
        ...
        public BulkInsertOrUpdate<T> UpdateOn(Expression<Func<T, object>> columnName)
        {
        ...
        void ITransaction.CommitTransaction(string connectionString, SqlCredential credentials)
        {
            ...
            {
                throw new InvalidOperationException("BulkInsertOrUpdate requires at least one UpdateOn column. This is usually the primary key of the table but can also be more than one column if you wish.");
            }
