...
namespace SqlBulkTools
{
    internal class BulkOperationsHelpers
    {
        ...
        private string GetVariableCharType(string column, string columnType,
        ...
        private string GetDecimalPrecisionAndScaleType(string column, string columnType,
        ...
        internal string BuildUpdateSet(HashSet<string> columns, string sourceAlias, string targetAlias,
            string identityColumn)
        {
            ...
            foreach (var column in columns.ToList())
            {
                ...
                {
                    if (column != "InternalId")
                        paramsSeparated.Add("[" + targetAlias + "]" + "." + "[" + column + "]" + " = " + "[" +
                                            sourceAlias + "]" + "." + "[" + column + "]");
                }
            }
        ...
        internal DataTable ToDataTable<T>(IEnumerable<T> items, HashSet<string> columns,
            Dictionary<string, string> columnMappings, List<string> matchOnColumns = null, bool? outputIdentity = null,
        ...
        internal void TryToOpenConnection(SqlConnection connection)
        {
            try
            {
                connection.Open();
        ...
        internal SqlConnection GetSqlConnection(string connectionName, SqlCredential credentials,
        ...
        internal void MapColumns(SqlBulkCopy bulkCopy, HashSet<string> columns,
        ...
        internal string GetOutputIdentityCmd(string identityColumn, bool outputIdentity, string tmpTableName,
            OperationType operation)
        {
            ...
            sb.Append("OUTPUT Source.InternalId, INSERTED." + identityColumn + " INTO " + tmpTableName +
                      "(InternalId, " + identityColumn + "); ");
        ...
        internal string GetIndexManagementCmd(string action, string table, HashSet<string> disableIndexList,
            bool disableAllIndexes = false)
        {
            ...
            var cmd = "DECLARE @sql AS VARCHAR(MAX)=''; " +
                      ...
                      " AND sys.objects.name = '" + table + "'" + (sb.Length > 0 ? sb.ToString() : "") +
                      "; EXEC(@sql);";
        ...
        internal DataTable GetDatabaseSchema(SqlConnection conn, string schema, string tableName)
        {
            ...
            if (dtCols.Rows.Count == 0 && schema != null)
                throw new InvalidOperationException("Table name '" + tableName + "\' with schema name \'" + schema +
        ...
        internal void InsertToTmpTable(SqlConnection conn, SqlTransaction transaction, DataTable dt,
            bool bulkCopyEnableStreaming, int? bulkCopyBatchSize, int? bulkCopyNotifyAfter, int bulkCopyTimeout,
        ...
        internal async Task InsertToTmpTableAsync(SqlConnection conn, SqlTransaction transaction, DataTable dt,
            bool bulkCopyEnableStreaming, int? bulkCopyBatchSize, int? bulkCopyNotifyAfter, int bulkCopyTimeout,
            SqlBulkCopyOptions sqlBulkCopyOptions)
        {
            using (var bulkcopy = new SqlBulkCopy(conn, sqlBulkCopyOptions, transaction))
