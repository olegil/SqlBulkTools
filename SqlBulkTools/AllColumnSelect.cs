...
namespace SqlBulkTools
{
    ...
    public class AllColumnSelect<T>
    {
        ...
        /// <summary>
        ///     A bulk insert will attempt to insert all records. If you have any unique constraints on columns, these must be
        ///     respected.
        ///     Notes: (1) Only the columns configured (via AddColumn) will be evaluated. (3) Use AddAllColumns to add all columns
        ///     in table.
        /// </summary>
        /// <returns></returns>
        ...
        ///     Any unique rows not found in target but exist in source will be added. Notes: (1) BulkInsertOrUpdate requires at
        ///     least
        ///     one MatchTargetOn property to be configured. (2) Only the columns configured (via AddColumn) (3) Use AddAllColumns
        ///     to add all columns in table.
        ///     will be evaluated.
        /// </summary>
        /// <returns></returns>
        public BulkInsertOrUpdate<T> BulkInsertOrUpdate()
        {
            return new BulkInsertOrUpdate<T>(_list, _tableName, _schema, _columns, _disableIndexList,
                _disableAllIndexes, _sourceAlias, _targetAlias,
                CustomColumnMappings, _sqlTimeout, _bulkCopyTimeout, _bulkCopyEnableStreaming, _bulkCopyNotifyAfter,
                _bulkCopyBatchSize, _sqlBulkCopyOptions, _ext);
        }

        /// <summary>
        ///     A bulk update will attempt to update any matching records. Notes: (1) BulkUpdate requires at least one
        ///     MatchTargetOn
        ///     property to be configured. (2) Only the columns configured (via AddColumn) will be evaluated. (3) Use AddAllColumns
        ///     to add all columns in table.
        /// </summary>
        /// <returns></returns>
        ...
        public BulkDelete<T> BulkDelete()
        {
            return new BulkDelete<T>(_list, _tableName, _schema, _columns, _disableIndexList, _disableAllIndexes,
                _sourceAlias, _targetAlias, CustomColumnMappings,
